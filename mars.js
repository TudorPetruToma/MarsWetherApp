const API_KEY = '4Na94GMJdg8MCEZVF10DyPQOrPfqO5y8HqOnS4yK'
...
const imperialRadio = document.getElementById('far')
...
function displayPreviousSols(sols){
    ...
    sols.forEach((solData, index) => {
        ...
        solContainer.querySelector('[data-temp-high]').innerText = displayTemperature
        (solData.maxTemp)
...
function displayTemperature(temperature){
    ...
    if(!isMetric()){
        returnTemperature = temperature * (5 / 9) + 32
    }
    return Math.round(returnTemperature)
}

function displaySpeed(wind){
    let returnSpeed = wind
    if(!isMetric()){
         returnSpeed = wind / 1.609
    }
...
function updateUnits(){
    ...
    speedUnits.forEach(unit => {
        unit.innerText = isMetric() ? 'kmh' : 'mph'
    })
